...
def run_linear(
    ...
    linear.load_state_dict({'weight': weights.T})
...
def run_multihead_self_attention(
    ...
    mha.load_state_dict({"q_proj.weight": q_proj_weight.T, "k_proj.weight": k_proj_weight.T, "v_proj.weight": v_proj_weight.T, "o_proj.weight": o_proj_weight.T}, strict=False)
...
def run_multihead_self_attention_with_rope(
    ...
    rope = RoPE(theta=theta, d_k=d_head, max_seq_len=max_seq_len)
    rotary_fn = lambda x: rope.forward(x, token_positions)
    return mha(in_features, rotary_fn)
